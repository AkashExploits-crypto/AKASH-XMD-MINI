...
export default async function initializeTelegramBot(manager) {
  // === CONFIG ===
  const ALLOWED_GROUP_ID = -1003868862476; // allowed group id
  const GROUP_INVITE_LINK = "https://t.me/AR_PUBLIC_HUB";

  // Token MUST come from env
  const BOT_TOKEN_TELEGRAM =
    process.env.BOT_TOKEN_TELEGRAM || process.env.BOT_TOKEN || "7989758503:AAFAzT6UmgfQUwQonEx6IymW7Qhj9UulFT8";
  ...
  async function handleCommand(msg) {
    try {
      ...
      if (cmd === "start") {
        ...
        if (!isAllowedGroup(msg)) return;
        return await tbot.sendMessage(msg.chat.id, `
🌷🌍 <b>𝐖ᴇʟᴄᴏᴍᴇ 𝐓ᴏ "Ꭺᴋꫝꜱʜ -𝐌ɪɴɪ/𝐌ᴅ"</b> 🦩🌼
●─────────────────●
<i>𝖦𝖾𝗇𝖾𝗋𝖺𝗍𝖾 𝗒𝗈𝗎𝗋 𝗉𝖺𝗂𝗋 𝖼𝗈𝖽𝖾 𝖿𝖺𝗌𝗍 & 𝗌𝖾𝖼𝗎𝗋𝖾𝗅𝗒.</i>
📌 <b>𝖴𝗌𝖺𝗀𝖾:</b> <code>/pair +91 700393888</code>

🌻 <i>𝖤𝗇𝗃𝗈𝗒 — 𝗌𝗍𝖺𝗒 𝖼𝗈𝗓𝗒 𝖺𝗇𝖽 𝗌𝖺𝖿𝖾! ☘</i>
●─────────────────●
`, {
  parse_mode: "HTML",
...
`, {
  ...
  reply_to_message_id: msg.message_id
        ...
        if (!digitsOnly) {
  return tbot.sendMessage(chatId, `
🐞 <b>𝐈ɴᴠᴀʟɪᴅ 𝐍ᴜᴍʙᴇʀ 𝐅ᴏʀᴍᴀᴛ</b>
●─────────────────●
<i>ᴘʟᴇᴀsᴇ ᴇɴᴛᴇʀ ᴅɪɢɪᴛs ᴀɴᴅ ʏᴏᴜʀ ᴄᴏᴜɴᴛʀʏ ᴄᴏᴅᴇ</i>

💐 <b>𝐄xᴀᴍᴘʟᴇ:</b> <code>/pair +91700393888</code>
●─────────────────●
`, {
  parse_mode: "HTML",
  reply_to_message_id: msg.message_id
...
? `🛑|| <b>𝐆ᴇɴᴇʀᴀᴛɪɴɢ 𝐏ᴀɪʀ 𝐂ᴏᴅᴇ</b> ||-
●─────────────────●
🎀🥲 <i>Country not detected — continuing anyway. Please include your country code next time for better results.</i>
●─────────────────●`
: `🛑|| <b>𝐆ᴇɴᴇʀᴀᴛɪɴɢ 𝐏ᴀɪʀ 𝐂ᴏᴅᴇ</b> ||-
●─────────────────●
${flag} <i>${escapeHtml(countryName)}</i>
🪄 <i>Please wait — creating your secure pairing...</i>
●─────────────────●`;
...
🎀 Ξ <b>Ꭺᴋꫝꜱʜ 𝗣𝗔𝗜𝗥𝗜𝗡𝗚</b> ⚡ Ξ
╭──────────────●●►
│ 🌻 <b>Number:</b> <code>${escapeHtml(rawArg)}</code>
│ 🕊️ <b>Code:</b> <code>${raw}</code>
│ 🌎 <b>Country:</b> ${detectedLine || "Unknown"}
╰───────────────●●►
●─────────────────●
🔐 <i>Settings › Linked Devices ›</i>
🍭 <i>Link A Device</i>\n
●─────────────────●\n
⚠️ <b>This code will expire quickly.</b>
<i>Please link your device immediately using the code above.</i>
`, {
  parse_mode: "HTML",
  reply_to_message_id: msg.message_id
});
        await tbot.sendMessage(chatId, `<pre>${escapeHtml(raw)}</pre>\n🌷 ${F("Ꭺᴋꫝꜱʜ - 𝐏ᴀɪʀ..")}`, { parse_mode: "HTML", reply_to_message_id: msg.message_id });
        return;
      }

      if (msg.chat && !isPrivate(msg) && isAllowedGroup(msg)) {
        // unknown command fallback in allowed group
        return tbot.sendMessage(msg.chat.id, `
🛑|| <b>𝐈ɴᴠᴀʟɪᴅᴇ 𝐂ᴏᴍᴍᴀɴᴅ</b> ||-
